...
"""
Compare proxai OpenAI protocol types against the official async-openai Rust crate (v0.40.2).
...
Comparison levels:
  ...
  2. Structural alignment: every proxai #[convert(from(...))] annotation is verified
     — struct fields match, enum variants match
"""
...
INTENTIONAL_EXCLUSIONS = {
    ...
    # ── Stream wrapper ──
    # Type alias for StreamResponse<ResponseStreamEvent>.  proxai has all the
    # SSE event types with #[convert] mappings but doesn't use the SDK's
    # generic stream wrapper — it handles SSE at the byte/event level.
...
CATEGORY_DESCRIPTIONS = {
    'conversation CRUD':
        'Types for conversation management endpoints. proxai handles items inline'
        ' via InputItem/OutputItem, no separate conversation CRUD.',
    'item list API':
        'Types for GET /v1/responses/{id}/items. proxai returns items inline'
        ' in the Response.',
    'compact API':
        'Types for POST /v1/responses/compact. proxai handles compaction data'
        ' shapes but does not expose the endpoint.',
    'token count API':
        'Types for POST /v1/responses/token-count.',
    'delete API':
        'Types for DELETE /v1/responses/{id}.',
    'stream wrapper':
        'SDK stream type alias StreamResponse<ResponseStreamEvent>. proxai'
        ' handles SSE at byte/event level.',
    'request body':
        'CreateResponse request-body type. proxai forwards raw JSON for'
        ' same-protocol routes; add when cross-protocol translation starts.',
    'dead code':
        'LocalShellOutput. Defined in async-openai but never referenced by'
        ' any other SDK type.',
    'legacy function':
        'Legacy function-calling types superseded by the tools system.',
    'shared utility':
        'Token-detail and format types in SDK shared/ module. proxai has'
        ' Responses-specific equivalents (InputTokenDetails, OutputTokenDetails,'
        ' TextResponseFormatConfiguration).',
    'chat request':
        'Chat Completions request/param/message types. proxai Responses-API-focused'
        ' and does not implement Chat Completions protocol.',
    'response detail':
        'Chat Completions response detail types.',
    'responses-protocol only':
        'Shared SDK types only present in Responses protocol. proxai has them'
        ...
        if _has_pub(child):
            if child.type == 'struct_item':
                ...
                if name:
                    ...
                    index[name] = dict(kind='struct', fields=fields,
                                       deprecated_fields=deprecated,
                                       line=child.start_point[0] + 1, file=rel)
            elif child.type == 'enum_item':
                name = _type_name(child)
                if name:
                    variants = _enum_variants_ast(child)
                    index[name] = dict(kind='enum', variants=variants,
                                       line=child.start_point[0] + 1, file=rel)
            elif child.type == 'type_item':
                name = _type_name(child)
                if name:
                    index[name] = dict(kind='type', line=child.start_point[0] + 1, file=rel)
...
def px_type_index(protocol='responses'):
    """Build dict of proxai type_name → {kind, fields, variants, file, line, convert_from}."""
    if protocol == 'responses':
...
def _index_px_types(node, buf, index, rel):
    for child in node.children:
        if _has_pub(child):
            ...
            # Extract convert annotation from preceding attribute nodes
            info['convert_from'] = _parse_convert(child, buf)
            ...
            if child.type == 'struct_item':
                ...
                if m:
                    name = m.group(1)
                    # Treat re-export as having a self-named convert target
                    # so it's counted as a StructuralConvert mapping
                    index[name] = dict(kind='struct', convert_from=f'openai::{name}',
                                       line=child.start_point[0] + 1, file=rel,
                                       is_re_export=True)
        _index_px_types(child, buf, index, rel)


def _parse_convert(node, buf):
    """Parse #[convert(from(openai[::module]::TypeName))] annotation.
    
    Walks backwards through all preceding attribute_item siblings.
    """
...
def _check_protocol(protocol, level=2):
    ...
    if SDK_PKG.exists():
        try:
            import tomllib
            sdk_ver = tomllib.loads(SDK_PKG.read_text()).get('package', {}).get('version', '')
        except Exception:
            pass
    ...
    px = px_type_index(protocol)     # name → {kind, fields, variants, file, line, convert_from}
    ...
    # ── Structural alignment via convert annotations ──────────
    ...
    unmatched_converts = []  # convert(from=X) but X not found in SDK index
    aligned_ok = 0

    for px_name, px_info in sorted(px.items()):
        convert_target = px_info.get('convert_from')
        if not convert_target:
            continue

        # Resolve SDK type name from convert target
        # convert format: "openai::TypeName" or "openai_mcp::TypeName" or "openai::module::TypeName"
        parts = convert_target.split('::')
        sdk_type_name = parts[-1]  # last part is the type name

        if sdk_type_name not in sdk:
            unmatched_converts.append((px_name, convert_target, sdk_type_name))
            continue

        sdk_info = sdk[sdk_type_name]

        if px_info['kind'] != sdk_info['kind']:
            struct_diffs.append((px_name, sdk_type_name,
        ...
        if px_info['kind'] == 'struct':
            ...
            if missing_f or extra_f or order_mismatch:
                struct_diffs.append((px_name, sdk_type_name, missing_f, extra_f,
                                     order_mismatch))
            else:
                aligned_ok += 1
            ...
            if missing_v or extra_v:
                enum_diffs.append((px_name, sdk_type_name, missing_v, extra_v))
            else:
                aligned_ok += 1
        else:
            aligned_ok += 1
    ...
    has_missing_fields = any(
        m_f for _, _, m_f, _, _ in struct_diffs
        if not isinstance(m_f, str)  # skip kind-mismatch entries
    )
    has_struct_gaps = has_missing_fields or bool(unmatched_converts)
    has_gaps = has_gaps or has_struct_gaps

    sdk_cnt = sum(1 for n, i in px.items() if i.get('convert_from'))

    # ── Level 1: header + summary ──────────────────────────────────
    hr()
    out(f"  OpenAI {label}  vs  async-openai {sdk_ver}")
    out(f"  SDK:  {', '.join(str(f.relative_to(SDK_DIR.parent.parent)) for f in _sdk_files(protocol))}")
    ...
    if level >= 2:
        ...
        out(f"  (SDK {len(sdk)} = matched {len(sk & pk)} + excluded {excluded_cnt}"
            f"{' + ignored ' + str(len(ignored_names)) if ignored_names else ''})")

    no_convert = [n for n, i in px.items() if not i.get('convert_from')]
    if level >= 2 and no_convert:
        out(f"  (of which {len(no_convert)} use manual `impl From` via serde_json round-trip"
            f" instead of StructuralConvert — SDK struct fields are private:"
            f" {', '.join(sorted(no_convert))})")
    out(f"  StructuralConvert mappings: {sdk_cnt}  |  Aligned: {aligned_ok}")

    deprecated_skipped = 0
    for px_name, px_info in px.items():
        ct = px_info.get('convert_from')
        if not ct:
            continue
        sdk_name = ct.split('::')[-1]
        if sdk_name in sdk:
            deprecated_skipped += len(sdk[sdk_name].get('deprecated_fields', set()))
    ...
    if level >= 2:
        # Type-level gaps
        if missing_types:
            h2(f"MISSING ({len(missing_types)}): SDK types not found in proxai ({label})")
        ...
        # Structural diffs
        if struct_diffs or enum_diffs or unmatched_converts:
            has_gaps = True
            if has_missing_fields:
                h2("Structural alignment (via #[convert(from(...))])")
            else:
                h2("Structural alignment — no missing fields (extras only, not gaps)")

            if unmatched_converts:
                out(f"\n  ⚠  Unresolved convert targets (SDK type not found):")
                for px_name, target, sdk_name in unmatched_converts:
                    out(f"      {px_name} → {target}  (❌ {sdk_name} not in SDK index)")

            if struct_diffs:
                for px_name, sdk_name, missing_f, extra_f, order_mismatch in struct_diffs:
            ...
            if not struct_diffs and not enum_diffs and not unmatched_converts:
                out(f"  ✅  All struct fields and enum variants match")
            out()

        # Intentional exclusions
        if excluded_types:
            h2(f"Intentional exclusions ({len(excluded_types)} types not needed in proxai)")
            # Group by category
            cat_order = sorted(set(INTENTIONAL_EXCLUSIONS.get(n, '') for n, _ in excluded_types))
            idx = 0
            for cat in cat_order:
                cat_types = [(n, i) for n, i in excluded_types
    ...
    if level >= 3 and px_extra:
        truly_extra = [(n, i) for n, i in px_extra if not i.get('convert_from')]
        if truly_extra:
            h2(f"Proxai-specific types ({len(truly_extra)} — no convert mapping to SDK)")
                ...
                try:
                    ...
                    if p.exists():
                        ...
                        if 'impl From' in ctx_text and 'serde_json' in ctx_text:
                            tag = 'manual From (private SDK fields)'
    ...
    if level >= 3 and ignored_names:
        h2(f"Ignored SDK types ({len(ignored_names)} — no proxai equivalent, silently skipped)")
