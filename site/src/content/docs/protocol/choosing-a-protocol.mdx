...
<FeatureGrid
  items={[
    { title: 'Same protocol pass-through', href: '/reference/protocols', icon: 'check', description: 'Use when the client and provider already speak the same protocol.' },
    { title: 'Responses client to Anthropic provider', href: '/using/recipes#route-openai-responses-to-an-anthropic-provider', icon: 'flow', description: 'Useful when a local OpenAI-compatible client targets an Anthropic Messages upstream.' },
    { title: 'Chat client to Anthropic provider', href: '/using/recipes#route-one-model-to-an-anthropic-provider', icon: 'route', description: 'Use explicit routes when only one model should cross protocols.' },
    { title: 'Anthropic client to Responses provider', href: '/reference/protocols', icon: 'protocol', description: 'Use only when the implemented pair preserves the behavior your client expects.' },
  ]}
/>
...
- [Routing and Providers](/using/routing-and-providers)
- [Protocols Reference](/reference/protocols)
- [Route Matching](/reference/route-matching)
