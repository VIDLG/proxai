...
每个 capture phase 的含义见 [观测与诊断](/zh/observability)。默认端点和其他运行时限制见 [默认值与限制](/zh/reference/defaults-and-limits)。MCP 仅是本地控制面，见 [行为契约 C5 和 C6](/zh/reference/behavior-contracts#监听分离)。
