...
其中 `translation/` 不参与 provider response bytes 观察和 provider-local normalization。provider streaming 处理仍然位于 `provider/` 边界内；`sse_translation.rs` 负责在 SSE bytes 与 `translation/` 的结构化 `StreamEvent` API 之间适配。
