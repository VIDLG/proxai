...
<ReferenceTable
  ...
  rows={[
    ...
    ['OpenAI Responses', '`ResponseStreamEvent`', '`response.created` / `response.in_progress` 初始化 stream；response terminal events 将 lifecycle 移入 terminal；pair translator 校验目标协议专属的 item/content ordering。'],
  ]}
/>
