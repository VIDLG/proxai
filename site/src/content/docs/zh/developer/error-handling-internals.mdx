---
title: 错误处理内部实现
description: ProxAI 错误处理模型——强类型错误、ErrorResponseSpec/Fields/Payload 三层结构、HTTP/SSE 错误响应复用、upstream error 保留
robots: noindex
---


import { ConversionPath, FeatureGrid, ProxaiCallout } from '@components/ProxaiDocs.jsx';
...
<ProxaiCallout type="tip" title="内部强类型，外部稳定投影">
  内部错误保留领域语义，便于代码分支和诊断；客户端响应保持紧凑、稳定、可读。
</ProxaiCallout>

## 错误投影链路

<ConversionPath
  from={{ title: 'Typed error', description: 'Core IngressError/TranslationError，以及应用 RequestError/InternalError/UpstreamError' }}
  via={[
    { title: 'error/render.rs', description: '把领域错误映射成 ErrorResponseSpec' },
    { title: 'ErrorResponseFields', description: 'HTTP/SSE 共用 status + payload 字段' },
  ]}
  to={{ title: '客户端响应', description: 'HTTP text/json response 或 SSE error event bytes' }}
/>
...
<FeatureGrid
  items={[
    { title: '配置说明', href: '/zh/using/configuration', icon: 'config', description: '通过 `[error_responses]` 选择 text 或 JSON 错误响应。' },
    { title: '观测与诊断', href: '/zh/using/observability', icon: 'terminal', description: '安全调试失败请求的 capture 和日志边界。' },
    { title: '常见排障', href: '/zh/using/troubleshooting', icon: 'alert', description: '面向用户的症状卡片和下一步检查。' },
  ]}
/>
...
```text
Core typed error
  JsonPayloadError / IngressError / TranslationError / StreamTranslationError
        ↓ 应用错误透明包装，保留 source chain
应用 typed error
...
Core 领域错误定义在 `crates/proxai-core` 的所属领域中；应用错误组合与响应投影定义在 `src/error/`：
...
这些类型描述的是 proxai 内部失败原因，而不是客户端响应格式。Core error 通过 `Result` 返回，应用错误使用透明 variant 保留其 source chain；合法但有损的适配才通过 `Observation` 发出，不应对同一个失败既 emit observation 又返回 error。错误应按领域分层，不要用宽泛转换把语义错误包装成 `std::io::Error`；`io::Error` 只用于真实 OS / filesystem IO。
...
`proxai-core::provider::normalize_provider_error` 负责把结构化 upstream 非 2xx JSON payload 归一化为：
...
应用层 `UpstreamResponseError` 继续负责 HTTP body bytes、空 body、非 JSON body 和未知 shape，并持有归一化后的 `ProviderError`。OpenAI `error.code` 和 Anthropic `error.type` 都会成为 provider error `code`；`code` / `param` 只在上游实际提供时保留，不伪造默认值。这实现了 [行为契约 C15 和 C18](/zh/reference/behavior-contracts#错误)。
...
body 输出完整错误对象的 pretty-printed JSON，但仍使用 `text/plain`，便于裸客户端阅读并保留上游 `code`/`param` 细节。
