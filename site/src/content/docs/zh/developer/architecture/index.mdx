...
- crates/
  - proxai-core/
    - src/
      ...
      - provider/ — 与 carrier 无关的 provider request preparation 与响应归一化
      - translation/ — 与 carrier 无关的跨协议转换

</FileTree>

应用 crate 在 `src/lib.rs` 中重新导出 `proxai_core::{protocol, routing, translation}`。HTTP ingress、provider projection/序列化、transport lookup 和 SSE carrier adaptation 留在 `src/`，结构化 ingress、provider 标签路由、provider request/response adaptation 与 translation 则位于 `proxai-core`。
  ...
  stages={[
    {
      ...
      responsibility: '读取 body bytes、检测协议、解析 JSON，再归一化和校验结构化请求。',
    },
    {
      name: '路由',
      codeName: false,
      modules: ['pipeline/provider_request.rs', 'proxai-core/routing/'],
      responsibility: '按协议和模型匹配 route，解析 provider 标签和 upstream model，再选择应用层 transport。',
    },
    {
      ...
      responsibility: '转换请求，在 core 中改写并适配结构化 provider value，再由应用层提取诊断 projection 并序列化 body。',
    },
    {
      name: '发送上游',
      codeName: false,
    ...
    },
    {
      ...
      responsibility: '将响应翻译回入站协议，在结构化事件与 SSE bytes 之间适配、观测失败并重建 HTTP 响应。',
    },
  ]}
/>
...
<ModuleGrid
  modules={[
    ...
    { name: 'src/pipeline/inbound.rs', icon: 'route', description: 'HTTP path 检测、JSON bytes 解析、应用错误映射和 request-scoped observer 接线。' },
    ...
    { name: 'proxai-core/provider/', icon: 'key', description: '与 carrier 无关的 provider request value preparation、响应 compatibility policy、结构化 response/event normalization 和类型化 adaptation observations。' },
    ...
    { name: 'sse_translation.rs', icon: 'flow', description: '应用层 carrier 适配器：把 SSE bytes 解析成 `StreamEvent`，调用 `ProviderNormalizer` 和纯 translator，保留失败现场，再编码回 SSE。' },
...
- `src/pipeline/inbound.rs` 是负责 path 检测、JSON bytes 解析、错误映射和 request-scoped observer 接线的应用 adapter。
- `pipeline/` 协调完整请求生命周期，并保持 phase 顺序显式。
- `proxai-core/provider/` 负责与 carrier 无关的 provider request value preparation、响应 compatibility policy、结构化 response/event normalization 和类型化 adaptation observations。
...
- `proxai-core/provider/` 是 provider 响应 compatibility 的结构化边界；应用 pipeline/SSE adapter 提供 HTTP carrier 与具体 observer。
...
- `sse_translation.rs` 与 `pipeline/` 组合 byte carrier、core provider normalization、纯 translator、诊断和客户端错误渲染。
...
`Translator` 是唯一公开的 façade，只持有一个 crate-private `TranslationContext`；该 context 原子地携带所选 route 与 observation hook。每个入口会派生一个绑定 phase 的 `TranslationScope`：request、non-streaming response 或 streaming response。Pair code 因此无需在 observation 调用中单独传 phase，也就不可能把一次操作的遥测误记到其他 pipeline phase。Value translation 借用派生出的 scope；structured stream translation 消费 `Translator`，把 streaming-response scope 移入返回的 stream，并在其中惰性创建私有、封闭的 `PairStreamingState`，其恰好包含六种跨协议状态机之一，或 identity 路径。这样 value-only translation 不会分配无用流状态，同时 pair state 也不可能被跨 stream 复用。Context 与 scope 定义位于 façade 下层的 `translation/context.rs`；顶层分发和 pair code 只接收 `&TranslationScope`，因此 dispatch、phase 与 observation 不可能对当前操作产生分歧。协议无关的 event、error、terminal、identity 与 lifecycle 类型集中在无外部依赖的 `translation/stream.rs`。Pair 只依赖 `context.rs` 与 `stream.rs`，`Translator` 再依赖这些底层和 pair state，所有依赖边保持单向。Carrier adapter 只调用 `Translator::translate_stream`，不再直接驱动 pair event 或 terminal handling。
