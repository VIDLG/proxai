...
<ModuleGrid
  modules={[
    ...
    { name: 'translation/', icon: 'protocol', description: '在显式协议 pair 之间做纯请求、响应与结构化事件转换，并暴露 `Translator` façade 和 `TranslationObserver` 扩展点。' },
...
- `translation/` 与 carrier 无关：`Translator` 只接收 JSON `Value` 或结构化 `StreamEvent`，不接收 HTTP response/body、SSE bytes 或 provider 私有结构。
...
- `translation/` 定义窄接口 `TranslationObserver`，只观测合法但有损的适配；`observe/` 提供应用实现，并负责日志、诊断和 capture。
...
- `translation/` 只依赖协议模型和结构化 value/event，不依赖 HTTP、SSE bytes、provider transport 或应用观测实现。
