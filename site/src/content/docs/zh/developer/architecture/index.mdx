...
应用 crate 在 `src/lib.rs` 中重新导出 `proxai_core::{protocol, translation}`，并保留一个委托给 `proxai_core::ingress` 的薄 `src/ingress/` adapter；这样应用侧路径保持稳定，而结构化 ingress 与 translation 逻辑真实落在 `proxai-core` 中。
