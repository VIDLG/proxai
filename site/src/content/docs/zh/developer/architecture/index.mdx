...
应用 crate 在 `src/lib.rs` 中重新导出 `proxai_core::{protocol, routing, translation}`。HTTP ingress、provider transport lookup 和 SSE carrier adaptation 留在 `src/`，结构化 ingress、provider 标签路由与 translation 则位于 `proxai-core`。
...
<ModuleGrid
  modules={[
    ...
    { name: 'proxai-core/provider/', icon: 'key', description: '与 carrier 无关的 provider compatibility policy 和结构化响应/event normalization。' },
...
- `proxai-core/provider/` 负责与 carrier 无关的 provider compatibility policy 和结构化响应/event normalization。
...
- `provider/` 负责应用侧 provider 请求渲染和 transport 细节，例如认证 header、上游 URL 和 idle-read timeout。
