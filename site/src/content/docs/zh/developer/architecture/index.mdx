...
应用 crate 在 `src/lib.rs` 中重新导出 `proxai_core::{protocol, translation}`，既保留现有应用侧模块路径，也让实现和依赖边界真实落在 `proxai-core` 中。
...
<ModuleGrid
  modules={[
    ...
    { name: 'proxai-core/translation/', icon: 'protocol', description: 'Core crate 在显式协议 pair 之间做请求、响应与结构化事件转换，并暴露 `Translator` façade 和 `TranslationObserver` 扩展点。' },
...
- `proxai-core/translation/` 定义窄接口 `TranslationObserver`，只观测合法但有损的适配；`observe/` 提供应用实现，并负责日志、诊断和 capture。
...
- `proxai-core/translation/` 只依赖协议模型和结构化 value/event，不依赖 HTTP、SSE bytes、provider transport 或应用观测实现。
