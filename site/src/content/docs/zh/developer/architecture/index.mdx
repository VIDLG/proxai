...
- src/
  ...
  - ingress/ — HTTP ingress adapter：path 检测、JSON bytes 解析、错误与 notice 映射
...
<ModuleGrid
  modules={[
    ...
    { name: 'src/ingress/', icon: 'route', description: 'HTTP path 检测、JSON bytes 解析、应用错误映射和 request-scoped observer 接线。' },
    ...
    { name: 'sse_translation.rs', icon: 'flow', description: '应用层 carrier 适配器：把 SSE bytes 解析成 `StreamEvent`，调用纯 translator，保留失败现场，再编码回 SSE。' },
...
- `src/ingress/` 是负责 path 检测、JSON bytes 解析、错误映射和 request-scoped observer 接线的应用 adapter。
...
- `sse_translation.rs` 是 upstream byte stream 与纯结构化事件 translator 之间的应用层适配器，负责保留触发失败的原始 frame 和 SSE 输出编码。
- `provider/` 负责 provider 请求渲染和 transport 细节，例如认证 header、上游 URL 和 idle-read timeout。
- `proxai-core/observe/` 定义共享 `Observer` contract 和封闭的 `Observation` enum。Core ingress 与 translation 发出类型化 variants；`src/observe/` 提供下游实现，并负责日志、诊断和 capture。
...
- `proxai-core/ingress/` 是归一化与校验的结构化 value 边界；`src/ingress/` 适配 HTTP bytes 和应用诊断。
...
- `sse_translation.rs` 与 `pipeline/` 组合 byte carrier、纯 translator、诊断和客户端错误渲染。
