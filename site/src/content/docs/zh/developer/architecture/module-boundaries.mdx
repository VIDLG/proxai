...
<ModuleGrid
  modules={[
    { name: 'src/pipeline/inbound.rs', icon: 'route', description: '从 HTTP path 检测入站协议，解析 JSON bytes，并在 core preparation 前映射 carrier 错误。' },
    { name: 'crates/proxai-core/src/ingress/', icon: 'route', description: '在 routing 和 translation 前归一化并校验结构化入站 payload。' },
    { name: 'crates/proxai-core/src/pipeline/', icon: 'flow', description: '组合 ingress、routing、translation、provider request adaptation 与 response processing 的 carrier-independent façade。' },
    ...
    { name: 'crates/proxai-core/src/translation/', icon: 'flow', description: '协议 pair 之间与 carrier 无关的 request/response/stream 转换；合法适配通过类型化 observation 报告。' },
    { name: 'crates/proxai-core/src/provider/', icon: 'file', description: '与 carrier 无关的 provider request payload preparation、compatibility policy、结构化响应/错误归一化和类型化 adaptation observations。' },
    { name: 'src/provider/*/request', icon: 'file', description: '应用层 provider projection/summary 提取、JSON 序列化和 request carrier 组装。' },
