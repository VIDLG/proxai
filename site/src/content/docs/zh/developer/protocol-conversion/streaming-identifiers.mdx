...
Chat 只区分 block 类型，加上一个整数（`chat_tool_index`），它必须在每个 `tool_calls[].index` chunk 上回填。实际内容——text 片段、tool 参数、reasoning text——逐 chunk emit，不需要保留。OpenAI-compatible `reasoning_content` 扩展可以为 Zed 等客户端承载普通 thinking 文本，但它没有 per-item identity 或 encrypted payload slot；因此 redacted thinking 仍会塌缩成 `Ignored`。
...
<ProtocolCompare
  ...
  rows={[
    ...
    ['RedactedThk', '`item_id` / `data`', '`Ignored`', '是', 'Responses 使用 `encrypted_content`；Chat `reasoning_content` 只能承载普通文本，不能承载 encrypted reasoning'],
