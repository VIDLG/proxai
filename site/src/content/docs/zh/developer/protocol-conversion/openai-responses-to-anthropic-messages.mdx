...
<ReferenceTable
  ...
  rows={[
    ...
    ['历史 `reasoning.content` / `summary`', 'trace 后跳过', 'Responses 未携带 Anthropic signature，不能构造成 Anthropic `thinking`。'],
    ['历史 `reasoning.encrypted_content`', '`redacted_thinking.data`', '为 Anthropic-originated 的 opaque thinking 历史原样保留；它不能与普通 thinking signature 互换。'],
  ]}
  codeFirstColumn={false}
/>
