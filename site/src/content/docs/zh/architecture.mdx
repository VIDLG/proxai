...
<FeatureGrid
  items={[
    { title: '配置', href: '/zh/configuration', icon: 'config', description: '运行时设置、路由、provider、capture、logging 和错误响应。' },
    { title: '协议总览', href: '/zh/protocols-overview', icon: 'protocol', description: '阶段轴、协议轴和当前支持的运行时路径。' },
    { title: '协议转换', href: '/zh/protocol-conversion', icon: 'flow', description: '面向协议 pair 的转换规则与协议类型对齐要求。' },
  ]}
/>
...
| 阶段 | 主要模块 | 职责 |
|---|---|---|
| `inbound_request` | `pipeline/inbound.rs`、`ingress/` | 读取 body、检测协议、归一化请求形态、构造 `InboundHttpFlow` |
| 路由 | `pipeline/inbound.rs`、`routing/` | 按协议和模型匹配 route，解析默认 provider |
| `provider_request` | `pipeline/provider_request.rs`、`provider/request`、`translation/request` | 转换请求 payload、provider 模型重写、body 序列化 |
| 发送上游 | `pipeline/provider_request.rs`、`provider/transport` | 构造认证 header、拼接上游 URL、通过 `reqwest` 发送 |
| `upstream_response` | `pipeline/upstream_response.rs`、`upstream/` | 读取状态码、header、非流式 body 或流式 body |
| `outbound_response` | `pipeline/provider_response.rs`、`translation/response`、`translation/streaming` | 将响应翻译回入站协议并重建 HTTP 响应 |
