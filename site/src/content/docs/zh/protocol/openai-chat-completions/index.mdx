...
`reasoning_content` 不属于官方 OpenAI Chat Completions OpenAPI schema，因此不会出现在 `src/protocol/openai/chat_completions/wire` 的 official wire structs 中。proxai 在 translation 边界从 Zed assistant 历史中提取该字段，并在生成非流式 message 或 SSE delta 后再注入。
