...
<ProtocolQuickFacts
  ...
  codeAreas={[
    'crates/proxai-core/src/protocol/openai/chat_completions/wire',
    'crates/proxai-core/src/protocol/openai/chat_completions/request',
    'crates/proxai-core/src/protocol/openai/chat_completions/response',
...
`reasoning_content` 不属于官方 OpenAI Chat Completions OpenAPI schema，因此不会出现在 `crates/proxai-core/src/protocol/openai/chat_completions/wire` 的 official wire structs 中。proxai 在 translation 边界从 Zed assistant 历史中提取该字段，并在生成非流式 message 或 SSE delta 后再注入。

当前 Zed 源码的行为是：assistant 历史使用 `reasoning_content`；流式响应同时接受 `delta.reasoning` 和 `delta.reasoning_content`，并将二者转换为 thinking events。proxai 出站统一使用 `reasoning_content`，以便 Zed 在下一轮 assistant history 中原样回放。这里没有 `reasoning_context` 字段。

Anthropic signature 和 redacted thinking 仍通过附加在 `reasoning_content` 后的 versioned continuation envelope 由客户端携带，proxai 不维护服务端 continuation 状态。
...
1. ingress 用 ProxAI 的 OpenAI-compatible typed parse 校验请求并提取 model。
