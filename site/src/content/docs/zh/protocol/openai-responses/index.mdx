...
运行时先由 ingress 解析入站 body，生成 `PreparedOpenaiResponsesRequest`。协议层的轻量投影是 `RequestProjection`，位于 `crates/proxai-core/src/protocol/openai/responses/request/projection.rs`。
