...
5. Strict 的同协议 SSE 响应保留原始 bytes 和 headers；`compatible` 模式会解析事件、调用 core `ProviderNormalizer`，再重新编码。`AnthropicSseObserver` 独立扫描上游事件，将 `MessageStreamEvent` 归纳到 `AnthropicResponseState`，并记录 completed/closed/error 日志。
