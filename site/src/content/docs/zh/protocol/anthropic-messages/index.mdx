...
struct Message {
    ...
    role: Role,
    type: MessageType,
    stop_reason: RequiredNullable<StopReason>,
    stop_sequence: RequiredNullable<String>,
    usage: Usage,
    ...
}
```

Required-nullable 响应字段使用 `RequiredNullable<T>`，并严格拒绝字段缺失。只有已测量到的 provider 兼容性偏差，才能在 protocol 反序列化之前由窄范围 normalization 补成显式 `null`；wire carrier 本身不会因此放宽。
...
5. Strict 的同协议 SSE 响应保留原始 bytes 和 headers；`compatible` 模式会解析事件、调用 core `normalize_provider_stream_event`，再重新编码。`AnthropicSseObserver` 独立扫描上游事件，将 `MessageStreamEvent` 归纳到 `AnthropicResponseState`，并记录 completed/closed/error 日志。
