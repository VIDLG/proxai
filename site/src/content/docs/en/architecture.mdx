...
import { FeatureGrid, ModuleGrid, ProxaiCallout } from '@components/ProxaiDocs.jsx';
...
<FeatureGrid
  items={[
    { title: 'Configuration', href: '/en/configuration', icon: 'config', description: 'Runtime settings, routes, providers, capture, logging, and errors.' },
    { title: 'Protocol overview', href: '/en/protocols-overview', icon: 'protocol', description: 'Phase axis, protocol axis, and supported runtime paths.' },
    { title: 'Protocol conversion', href: '/en/protocol-conversion', icon: 'flow', description: 'Pair-oriented conversion rules and protocol alignment expectations.' },
  ]}
/>
...
|---|---|---|
| `inbound_request` | `pipeline/inbound.rs`, `ingress/` | Read body, detect protocol, normalize request shape, create `InboundHttpFlow`. |
| Routing | `pipeline/inbound.rs`, `routing/` | Match by protocol and model, resolve default provider. |
| `provider_request` | `pipeline/provider_request.rs`, `provider/request`, `translation/request` | Translate request payload, rewrite provider model, serialize body. |
| Send upstream | `pipeline/provider_request.rs`, `provider/transport` | Build auth headers, construct upstream URL, send via `reqwest`. |
| `upstream_response` | `pipeline/upstream_response.rs`, `upstream/` | Read status, headers, and body or stream. |
| `outbound_response` | `pipeline/provider_response.rs`, `translation/response`, `translation/streaming` | Translate response back to inbound protocol and rebuild HTTP response. |
