...
| `--upstream` | Temporarily override the `openai_responses` default provider's upstream URL for this run |
| `--api-key` | Temporarily override the `openai_responses` default provider's API key for this run |
...
For what each capture phase means, see [Observability](/en/observability). For default endpoints and other runtime limits, see [Defaults and Limits](/en/reference/defaults-and-limits). MCP is a local-only control surface — see [Behavior Contract C5 and C6](/en/reference/behavior-contracts#listener-separation).
