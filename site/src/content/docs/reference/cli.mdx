...
For what each capture phase means, see [Observability](/using/observability). For default endpoints and other runtime limits, see [Defaults and Limits](/reference/defaults-and-limits). MCP is a local-only control surface — see [Behavior Contract C5 and C6](/reference/behavior-contracts#listener-separation).
