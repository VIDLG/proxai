...
Single reference for what clients see when ProxAI fails: payload fields, allowed `type` values, HTTP status semantics, and preserved upstream headers. For the internal projection pipeline, see [Error Handling Internals](/developer/error-handling-internals).
...
- `text`: the full error object is returned as readable pretty-printed JSON with `text/plain`. Default for Zed readability while preserving upstream `code`/`param` details.
- `json`: the full wrapped JSON above is returned as compact `application/json`.
...
It does not fabricate these headers. Response `Content-Type` is decided by the outbound protocol, not passed through from the upstream — see [Contract C13](/reference/behavior-contracts#response-reconstruction). Upstream headers are filtered before forwarding — see [Contract C14](/reference/behavior-contracts#response-reconstruction).
