...
<ReferenceTable
  columns={['Protocol value', 'Wire protocol', 'Request path']}
  rows={[
...
Each new pair must add runtime routing, request/response conversion, streaming conversion, and focused tests. The routing rules implement [Behavior Contract C7–C10](/reference/behavior-contracts#routing-and-conversion).

## Module naming convention

```text
crates/proxai-core/src/translation/<inbound_protocol>/to_<provider_protocol>/
```

Examples:

```text
crates/proxai-core/src/translation/openai_responses/to_anthropic_messages/
crates/proxai-core/src/translation/anthropic_messages/to_openai_responses/
crates/proxai-core/src/translation/openai_chat_completions/to_anthropic_messages/
```

For the conversion design rules, see [Protocol Conversion](/developer/protocol-conversion).
