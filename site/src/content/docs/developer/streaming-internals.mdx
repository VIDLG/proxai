...
Raw chunks, triggering SSE frames, HTTP metadata, diagnostics, and client error rendering remain in the application layer. The core returns semantic failures through `Stream<Item = Result<StreamEvent, StreamTranslationError>>`; `TranslationObserver` is reserved for legal-but-lossy adaptations and is not an error channel.
