...
<ReferenceTable
  ...
  rows={[
    ...
    ['OpenAI Responses', '`ResponseStreamEvent`', '`response.created` / `response.in_progress` initializes the stream; response terminal events move the lifecycle to terminal; pair translators validate target-specific item/content ordering.'],
  ]}
/>
...
├── mod.rs       # routes source events through pair state and owns carrier timing
...
- **pair mod.rs** dispatches source events, invokes pair state, handles terminal outcomes, and maps typed target events to carrier-level `StreamEvent`s.
