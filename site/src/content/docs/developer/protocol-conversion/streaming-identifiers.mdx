...
as it arrives and never needs to be remembered. The OpenAI-compatible
`reasoning_content` extension carries plain thinking text for clients such as
Zed, but it has no per-item identity or encrypted payload slot; redacted
thinking therefore still collapses into `Ignored`.
...
<ProtocolCompare
  ...
  rows={[
    ...
    ['RedactedThk', '`item_id` / `data`', '`Ignored`', 'yes', 'Responses `encrypted_content`; Chat `reasoning_content` only carries plain text, not encrypted reasoning'],
