...
has to feed back on every `tool_calls[].index` chunk. Visible text, tool
arguments, and ordinary reasoning are emitted incrementally. For Anthropic
thinking continuation, however, ProxAI accumulates signatures and redacted
payloads until the relevant blocks stop, then appends a versioned continuation
envelope in a final `reasoning_content` delta. The client, rather than the
proxy, holds that envelope for a later replay.
...
but must surface its `data` payload as `encrypted_content`. These are the official
Responses lifecycle requirements. On ingress, the Bedrock Mantle compatibility
channel `response.reasoning.delta/done` may omit item and output coordinates;
ProxAI keeps that channel explicitly unscoped instead of inventing identities.
...
<ProtocolCompare
  ...
  rows={[
    ...
    ['', '`item_id`', 'no', 'yes', 'Official Responses events mandate `item_id`; the inbound Mantle reasoning compatibility channel is explicitly unscoped'],
    ...
    ['RedactedThk', '`item_id` / `data`', 'yes, client-carried continuation', 'yes', 'Responses uses `encrypted_content`; Chat appends a versioned envelope to `reasoning_content` for compatible history replay'],
