...
- accumulate `signature_delta` and `redacted_thinking` payloads, then append a versioned ProxAI continuation envelope in the final `reasoning_content` delta. A compatible client replays it in assistant history, and ProxAI strips it before forwarding to Anthropic;
