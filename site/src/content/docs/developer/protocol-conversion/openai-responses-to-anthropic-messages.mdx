...
<ReferenceTable
  ...
  rows={[
    ...
    ['Historical `reasoning.content` / `summary`', 'Skipped with trace', 'They cannot become Anthropic `thinking` because Responses does not carry an Anthropic signature.'],
    ['Historical `reasoning.encrypted_content`', '`redacted_thinking.data`', 'Preserved verbatim for Anthropic-originated opaque thinking history; it is not interchangeable with a normal thinking signature.'],
  ]}
  codeFirstColumn={false}
/>
