...
> This is the developer entry point for `crates/proxai-core/src/translation/`. If you are looking for user-facing protocol behavior, start with [Protocol Guide](/protocol).

## Boundaries

- `crates/proxai-core/src/protocol/` owns protocol-specific Rust wire models.
- `src/pipeline/inbound.rs` parses HTTP body bytes and maps application errors/notices; `crates/proxai-core/src/ingress/` owns structured normalization and validation before translation.
- `crates/proxai-core/src/provider/` owns carrier-independent provider request value preparation, structured response/error normalization, and typed provider adaptation observations.
- `crates/proxai-core/src/translation/` owns pure cross-protocol conversion between an inbound `request_protocol` and an outbound provider `protocol`.
- `src/provider/request.rs` owns application-side projection/summary extraction, JSON body serialization, and provider request carrier assembly.
...
Do not hide general cross-protocol conversion inside a provider subtree. Structured provider quirks belong in `crates/proxai-core/src/provider/`, HTTP carrier adaptation stays in the application, and protocol-to-protocol shape changes belong in `crates/proxai-core/src/translation/`.
...
Do not pass HTTP `Response`, `Body`, provider request structs, or route/model rewrite details into `crates/proxai-core/src/translation/`. This is the carrier-boundary purity required by [Behavior Contract C26](/reference/behavior-contracts#translation-purity).
...
<ConversionPath
  ...
  via={[
    ...
    { title: 'Provider', description: 'core request adaptation + application projection, serialization, auth, and transport' },
...
- Responses `reasoning.effort` maps to Anthropic `output_config.effort` for
  supported effort values (`low`, `medium`, `high`, `xhigh`, `max`). This is the
...
<FeatureGrid
  items={[
    { title: 'Message placement', icon: 'route', href: '/developer/protocol-conversion/message-placement', description: 'System/developer instructions, user content, tool calls, tool results, choices, and stream terminators.' },
    { title: 'Refusal and status mapping', icon: 'alert', href: '/developer/protocol-conversion/refusal-and-status', description: 'How refusal content and terminal state map between Anthropic, Responses, and Chat.' },
    { title: 'Anthropic Messages to Responses', icon: 'flow', href: '/developer/protocol-conversion/anthropic-messages-to-openai-responses', description: 'Content blocks, tool_use, thinking, stop reasons, and stream events into Responses output items.' },
    { title: 'Chat to Anthropic Messages', icon: 'flow', href: '/developer/protocol-conversion/openai-chat-completions-to-anthropic-messages', description: 'Messages, tools, streaming chunks, finish reasons, and lossy identity behavior for Chat clients.' },
    { title: 'Responses to Anthropic Messages', icon: 'flow', href: '/developer/protocol-conversion/openai-responses-to-anthropic-messages', description: 'Request, response, streaming, tool, reasoning, and stop conversion for the highest-value pair.' },
    { title: 'SDK alignment and compatibility', icon: 'check', href: '/developer/protocol-conversion/sdk-alignment', description: 'Required-nullable fields, TypeScript SDK comparison, and measured compatibility repairs.' },
    { title: 'Streaming identifiers', icon: 'flow', href: '/developer/protocol-conversion/streaming-identifiers', description: 'Per-stream index, item_id, event granularity, snapshot-bound state, and parallel assembly.' },
  ]}
/>
...
2. Update the relevant protocol document under `site/src/content/docs/` (English, root locale) and `site/src/content/docs/zh/` if behavior changes for users or examples.
