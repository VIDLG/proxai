...
- src/
  ...
  - provider/ — provider request projection/serialization, response observation, and HTTP transport
  - upstream/ — upstream response reading
  - pipeline/ — typed proxy pipeline, including HTTP ingress adaptation
  - observe/ — capture, logging, diagnostics
  - mcp/ — MCP control surface
- crates/
  - proxai-core/
    - src/
      - ingress/ — carrier-independent inbound normalization and validation
      - protocol/ — protocol wire types and protocol enums
      - routing/ — compiled protocol/model routing and provider-label selection
      - provider/ — carrier-independent provider request preparation and response normalization
      - translation/ — carrier-independent cross-protocol conversion

</FileTree>

The application crate re-exports `proxai_core::{protocol, routing, translation}` from `src/lib.rs`. It keeps HTTP ingress, provider projections/serialization, transport lookup, and SSE carrier adaptation in `src/`, while structured ingress, provider-label routing, provider request/response adaptation, and translation stay in `proxai-core`.
  ...
  stages={[
    {
      ...
      modules: ['src/pipeline/inbound.rs', 'proxai-core/ingress/'],
      responsibility: 'Read body bytes, detect protocol, parse JSON, then normalize and validate the structured request.',
    },
    {
      name: 'Routing',
      codeName: false,
      modules: ['pipeline/provider_request.rs', 'proxai-core/routing/'],
      responsibility: 'Match by protocol and model, resolve a provider label and upstream model, then select the application transport.',
    },
    {
      name: 'provider_request',
      modules: ['pipeline/provider_request.rs', 'proxai-core/translation/request', 'proxai-core/provider/request', 'provider/request'],
      responsibility: 'Translate the request, rewrite and adapt the structured provider value in core, then project diagnostics and serialize the body in the application.',
    },
    {
      name: 'Send upstream',
      codeName: false,
...
<ModuleGrid
  modules={[
    ...
    { name: 'src/pipeline/inbound.rs', icon: 'route', description: 'HTTP path detection, JSON byte parsing, application error mapping, and request-scoped observer wiring.' },
    { name: 'proxai-core/ingress/', icon: 'route', description: 'Carrier-independent structured request normalization, validation, model extraction, and typed ingress observations.' },
    { name: 'proxai-core/observe/', icon: 'gauge', description: 'Shared `Observer` contract, closed `Observation` variants, and no-op default; no concrete sinks.' },
    { name: 'proxai-core/routing/', icon: 'flow', description: 'Carrier-independent provider-label selection from request protocol, model pattern, defaults, and route configuration.' },
    { name: 'proxai-core/provider/', icon: 'key', description: 'Carrier-independent provider request value preparation, response compatibility policy, structured response/event normalization, and typed adaptation observations.' },
    { name: 'proxai-core/translation/', icon: 'protocol', description: 'Core crate request, response, and structured-event conversion across explicit protocol pairs. Exposes the `Translator` façade and emits typed observations through the shared `Observer`.' },
    { name: 'provider/', icon: 'key', description: 'Provider projection/summary extraction, request serialization, response observation, auth headers, upstream URL construction, and transport.' },
    { name: 'upstream/', icon: 'terminal', description: 'Reading upstream status, headers, complete bodies, or streaming byte carriers.' },
    { name: 'sse_translation.rs', icon: 'flow', description: 'Application carrier adapter: parses SSE bytes into `StreamEvent`, applies `ProviderNormalizer`, invokes the pure translator, retains failure context, and encodes output back to SSE.' },
...
- `src/pipeline/inbound.rs` is the application adapter for path detection, JSON byte parsing, error mapping, and request-scoped observer wiring.
- `pipeline/` coordinates the full request lifecycle and owns phase order.
- `proxai-core/provider/` owns carrier-independent provider request value preparation, response compatibility policy, structured response/event normalization, and typed adaptation observations.
...
flowchart TD
  ...
  pipeline --> core_ingress[proxai-core/ingress/]
  pipeline --> core_provider[proxai-core/provider/]
  pipeline --> routing[proxai-core/routing/]
...
- `proxai-core/provider/` is the structured provider-response compatibility boundary; application pipeline/SSE adapters supply HTTP carriers and concrete observers.
...
- `sse_translation.rs` and `pipeline/` compose byte carriers, core provider normalization, the pure translator, diagnostics, and client-facing error rendering.
