...
The application crate re-exports `proxai_core::{protocol, translation}` from `src/lib.rs` and keeps a thin `src/ingress/` adapter over `proxai_core::ingress`, preserving application-side paths while keeping structured ingress and translation logic in `proxai-core`.
