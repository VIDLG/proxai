...
- src/
  ...
  - ingress/ — HTTP ingress adapter: path detection, JSON byte parsing, error/notice mapping
  ...
  stages={[
    {
      ...
      modules: ['pipeline/inbound.rs', 'src/ingress/', 'proxai-core/ingress/'],
...
<ModuleGrid
  modules={[
    ...
    { name: 'src/ingress/', icon: 'route', description: 'HTTP path detection, JSON byte parsing, application error mapping, and request-scoped observer wiring.' },
...
- `src/ingress/` is the application adapter for path detection, JSON byte parsing, error mapping, and request-scoped observer wiring.
...
- `proxai-core/observe/` defines the shared `Observer` contract and closed `Observation` enum. Core ingress and translation emit typed variants; `src/observe/` supplies the downstream implementation and owns logging, diagnostics, and capture.
...
flowchart TD
  ...
  pipeline --> app_ingress[src/ingress/]
  ...
  app_ingress --> core_ingress[proxai-core/ingress/]
...
- `proxai-core/ingress/` is the structured value boundary for normalization and validation; `src/ingress/` adapts HTTP bytes and application diagnostics.
