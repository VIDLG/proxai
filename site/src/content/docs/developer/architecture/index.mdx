...
<PipelineStageCards
  stages={[
    ...
    },
    {
      ...
      modules: ['pipeline/provider_response.rs', 'sse_translation.rs', 'translation/response', 'translation/streaming'],
...
<ModuleGrid
  modules={[
    ...
    { name: 'translation/', icon: 'protocol', description: 'Pure request, response, and structured-event conversion across explicit protocol pairs. Exposes the `Translator` façade and `TranslationObserver` extension point.' },
...
- `translation/` is carrier-independent: `Translator` accepts JSON `Value`s or structured `StreamEvent`s and never receives HTTP responses, bodies, SSE bytes, or provider-private structs.
...
- `translation/` defines the narrow `TranslationObserver` trait for legal-but-lossy adaptation telemetry; `observe/` supplies the application implementation and owns logging, diagnostics, and capture.
...
- `translation/` depends only on protocol models and structured values/events; it does not depend on HTTP, SSE bytes, provider transport, or application observation.
