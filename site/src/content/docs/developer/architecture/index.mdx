...
The application crate re-exports `proxai_core::{protocol, routing, translation}` from `src/lib.rs`. It keeps HTTP ingress, provider transport lookup, and SSE carrier adaptation in `src/`, while structured ingress, provider-label routing, and translation stay in `proxai-core`.
...
<ModuleGrid
  modules={[
    ...
    { name: 'proxai-core/provider/', icon: 'key', description: 'Carrier-independent provider compatibility policy and structured response/event normalization.' },
...
- `proxai-core/provider/` owns carrier-independent provider compatibility policy and structured response/event normalization.
...
- `provider/` owns application-side provider request rendering and transport details such as auth headers, upstream URLs, and idle-read timeout.
