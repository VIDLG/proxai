...
Anthropic Messages 的 no-conversion 转发，也支持通过显式路由把 OpenAI
Chat Completions 转换为 Anthropic Messages，供 Anthropic-compatible
provider 使用。配置模型已经按协议感知整理好，后续可以显式扩展到更多转换
路径与路由，但不会因此演变成泛化的 AI 网关。
