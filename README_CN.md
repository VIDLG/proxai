...
更合适的兼容方式不是把 `system` 改成 `user`，而是把 system 文本移动到顶层 `instructions`，并从 `input` 中删除原 system message。sub2api 的根因分析和上游 issue 草稿见 [docs/sub2api-system-role-issue.md](docs/sub2api-system-role-issue.md)。
...
workflow 会在每次 push 和 pull request 时构建 Windows exe。推送 `v0.1.0` 这种 tag 时，还会发布 GitHub Release asset：
...
详细的 sub2api 分析和 issue 草稿放在 [docs/sub2api-system-role-issue.md](docs/sub2api-system-role-issue.md)。简短来说，OpenAI OAuth Codex transform 应该把 Responses `content` part 中的 `type:"input_text"` 按 system 文本处理：提取到顶层 `instructions`，并从 `input` 中移除原 system message。
