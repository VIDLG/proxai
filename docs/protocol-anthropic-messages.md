...
proxai 在这条链路中不重组这些事件。当前 Anthropic provider 只保留原始 SSE bytes、capture 可配置内容，并用 `SseScanner` 识别 terminal event，用于 completed/closed/error 日志。
...
- 当前 proxai 不解析 Anthropic SSE 为 `MessageStreamEvent` 后再重新编码，也不对 Anthropic 工具参数流做超时注入；这些类型主要用于后续更细粒度观察和跨协议转换。
...
2. translation 构造 `ForwardedRequest::AnthropicMessages`，保留原始 body。
3. provider 直接转发到上游。
4. 非流式响应原样返回并可 capture。
5. SSE 响应保留原始 bytes 和 headers；`AnthropicSseObserver` 使用通用 `SseScanner` 识别 terminal event 并记录 completed/closed/error 日志。

当前 Anthropic observer 不做深度事件重建，也不注入工具参数超时诊断。协议层的 `MessageStreamEvent` 已为后续更细粒度观察、摘要和跨协议翻译准备好类型。
